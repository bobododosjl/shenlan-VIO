...
// euler2Rotation:   body frame to inetial frame
...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        imupose_mean.imu_gyro = (imupose_pre.imu_gyro - imupose_pre.imu_gyro_bias + imupose_now.imu_gyro - imupose_now.imu_gyro_bias) / 2.0;

        //delta_q = [1 , 1/2 * thetax , 1/2 * theta_y, 1/2 * theta_z]
        Eigen::Quaterniond dq;
        Eigen::Vector3d dtheta_half =  (imupose_mean.imu_gyro - imupose_mean.imu_gyro_bias) * dt /2.0;
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
