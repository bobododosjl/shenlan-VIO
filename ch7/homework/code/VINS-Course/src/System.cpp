    ...
    while (bStart_backend)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
            {
                ...
                ofs_pose << fixed << dStamp << " " << p_wi.transpose()[0] << " " << p_wi.transpose()[1] << " "<< p_wi.transpose()[2] << " " << q_wi.coeffs().transpose().x() << " " << q_wi.coeffs().transpose().y() << " " << q_wi.coeffs().transpose().z() << " " << q_wi.coeffs().transpose().w() << endl;
            }
        }
        m_estimator.unlock();
    }
